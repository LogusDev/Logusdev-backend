  ...
  export const uploadFotoGuincheiro = async (req, res) => {
    try {
      ...
      const objectName = `guincheiros/${email.replace(/[@.]/g, '_')}_${Date.now()}${path.extname(file.originalname)}`;
      ...
      const fotoUrl = `http://${process.env.IP_MINIO}:9000/users/${objectName}`;
